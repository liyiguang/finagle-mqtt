...
abstract class AbstractMQTTMessageDecoder extends FrameDecoder with StateMachine{
  ...
  case class ReadDataLength(header:MQTTHeader,lengthDecoder:LengthDecoder) extends State
  case class ReadDate(header:MQTTHeader,length:Int) extends State
  ...
  override def decode(ctx: ChannelHandlerContext, channel: Channel, buffer: ChannelBuffer): AnyRef = {
    state match {
      case ReadHeader() =>
        val header = new MQTTHeader(buffer.readByte())
